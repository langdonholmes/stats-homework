...
Given that $\hat{\omega}^{2}_{<parent \times condition>} = 0.03$, we know that $\sigma^{2}_{<parent \times condition>} = \frac{3\sigma^2}{97}$. $\omega^{2}_{condition} = 0.06$, so we can plug in the above definitions of $\sigma^2_{parent}$ and $\sigma^{2}_{<parent \times condition>}$ to the equation for the total variance, which we can solve for $\sigma^2_{condition} = \frac{5991\sigma^2}{86621}$. Finally, we plug this in to the equation for the complete omega squared, which gives us $\omega^{2}_{condition} = 0.065$.
...
1. 51, the main effect of A is +24 with no interaction.
2. -9, the main effect of A is +24 with no interaction.
